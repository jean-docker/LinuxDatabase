 ...
 * @return
 */
BPlusTreeNode *BPlusTree::parent(BPlusTreeNode *root, const BPlusTreeNode *current_node, int &p_index){
    ...
    for (int index = 0; index < root->child_size; ++index) {
        if(root->childs[index] == current_node){
           p_index = index;
            ...
            else if (root->childs[index]->index_nodes[0].value > current_node->index_nodes[current_node->index_node_size-1].value) {
                break;
            }
